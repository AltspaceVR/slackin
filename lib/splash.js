...
export default function splash ({ css, path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash',
    !iframe && dom('.logos',
      logo && dom('.logo.org'),
      dom('.logo.slack')
    ...
    style({ logo, active, large, iframe }),
      css && css.map(c => dom('link rel=stylesheet', { href: c })),
    // xxx: single build
    dom('script', `
      data = {};
